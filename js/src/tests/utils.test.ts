...
import { Vault, TFile, Notice, RequestUrlParam } from "obsidian";
...
describe("process result logic", () => {
    ...
    test("sends notice on success result and successful save of result and copies to clipboard", async () => {
        ...
        window.alert = jest.fn((msg: string) => { console.log("alert: " + msg) });
        mockNotice.mockImplementationOnce((msg: string) => { console.log("notice: " + msg); return new Notice(msg) });
...
describe("get slack message logic", () => {
    ...
    test("successfully validates inputs and processes result", async () => {
        ...
        let cookie = "a cookie";
        let url = "a url"
        let get_slack_message_func = (apiToken: string, cookie: string, url: string, feature_flags: any, request_func: (params: RequestUrlParam) => Promise<string>): Promise<string> => {
            return Promise.resolve("resolve");
        }
        let settings = utils.DEFAULT_SETTINGS
        let mock_vault = new Vault();

        window.alert = jest.fn((msg: string) => { console.log("alert2: " + msg) });
        jest.spyOn(utils, "process_result").mockImplementationOnce(() => { return Promise.resolve() })

        await utils.get_slack_message_modal_on_close_helper(api_token, cookie, url, get_slack_message_func, settings, mock_vault)
